...
namespace TriviaQuizGame
{
    ...
    /// Este script controla el juego, lo inicia, sigue el progreso del juego y lo termina con el fin del juego.
    /// </summary>
    public class TQGGameController : MonoBehaviour
    {
        //////////////////////////////
        //private static string database = "ulearnet_reim_pruebas";
        public Text text;
        public Counter counter;
        private int IdElemento = 290034;
        ...
        // El turno actual del jugador. 0 significa que es el turno del jugador 1, 1 significa que es el turno del jugador 2, etc.
        internal int currentPlayer = 0;

        // Is this game played in hot-seat mode? This mode lets each player answer a question in turn.
        // ¿Este juego se juega en modo hot-seat? Este modo permite que cada jugador responda una pregunta por turno.
        ...
        //"El número de jugadores que participan en el partido. Este número no puede ser mayor que el número total de jugadores en la lista (los que asignas un scoreText / livesBar)"
        ...
        // El objeto que muestra la pregunta actual
        ...
        // Los objetos de imagen y video dentro del objeto de pregunta
        internal RectTransform imageObject;
        internal GameObject videoObject;

        //Una lista de todas las posibles preguntas del juego. Cada pregunta tiene una cantidad de respuestas correctas / incorrectas, un texto de seguimiento, un valor adicional, tiempo y también puede tener una imagen / video como fondo de la pregunta.
        ...
        //El índice de la pregunta actual que se está formulando. -1 es el índice de la primera pregunta, 0 es el índice de la segunda, y así sucesivamente
        internal int currentQuestion = -1;

        // Is a question being asked right now?
        //¿Se está haciendo una pregunta en este momento?
        internal bool askingQuestion;


        //Aleatorice la lista de preguntas. Use esto si no quiere que las preguntas aparezcan en el mismo orden cada vez que juegue. Combine esto con 'sortQuestions' si desea que las preguntas sean aleatorias dentro de los grupos de bonificación.
        ...
        //Aleatorizar el orden de visualización de las respuestas cuando se presente una nueva pregunta
        ...
        // La respuesta seleccionada actualmente cuando se usa ButtonSelector
        internal int currentAnswer = 0;

        // Holds the answers when shuffling them
        // Mantiene las respuestas cuando las baraja
        internal string[] tempAnswers;


        //Ordene la lista de preguntas desde la bonificación más baja hasta la más alta y colóquelas en grupos. Use esto si desea que las preguntas se muestren de la más fácil a la más difícil (la dificultad de una pregunta se decide por el valor de bonificación que le da
        ...
        //Evite que un cuestionario repita preguntas.Una vez que se hayan formulado todas las preguntas de un cuestionario, se repetirán nuevamente.
        ...
        //Cuántas preguntas del grupo de bonificación actual deben hacerse antes de pasar al siguiente grupo.Si no clasificamos las preguntas por grupos de bonificación, este valor se ignora.Si hay varios jugadores en el juego, el valor de las preguntas por grupo se multiplicará por el número de jugadores, de modo que cada uno pueda tener la oportunidad de responder una pregunta del mismo grupo antes de pasar al siguiente grupo.
        ...
        // La pestaña de progreso y los objetos de texto que muestran en qué pregunta estamos. La pestaña también muestra si respondimos una pregunta correctamente o no
        ...
        // El tamaño de la pestaña de progreso. Esto se calcula automáticamente y se utiliza para alinear la barra de progreso con el centro.
        ...
        //Limite el número total de preguntas formuladas, independientemente de si contestamos correctamente o no.Úselo si desea que se hagan un número estricto de preguntas en el juego(por ejemplo, 10 preguntas). Si lo mantiene en 0, el número de preguntas no estará limitado y pasará por todos los grupos de preguntas del cuestionario antes de terminarlo.
        ...
        // El número total de preguntas que hicimos. Esto se usa para verificar si alcanzamos el límite de preguntas.
        internal int questionLimitCount = 0;


        // La cantidad de preguntas que respondimos correctamente. Esto se usa para mostrar el resultado al final del juego.
        ...
        //El número máximo de errores permitidos.Si comete muchos errores, pierde una vida y pasa a la siguiente pregunta.
        ...
        // Cuántos segundos quedan antes de que acabe el juego
        ...
        //Si configuramos este tiempo por encima de 0, anulará los tiempos individuales para cada pregunta. El tiempo global no se reinicia entre preguntas
        ...
        /// 
        /// Start solo se llama una vez durante la vida del comportamiento.
        /// La diferencia entre Awake y Start es que Start solo se llama si la instancia de script está habilitada.
        ...
        void Start()
        {
            ...
            // Desactivar multitouch para que no toquemos dos respuestas al mismo tiempo (evita las trampas de múltiples respuestas, gracias a Miguel Paolino por detectar este error)
            Input.multiTouchEnabled = false;

            // Cache the current event system so we can enable and disable it between questions
            // Guarde en caché el sistema de eventos actual para que podamos habilitarlo y deshabilitarlo entre preguntas
            ...
            // Si el cuestionario se está ejecutando en una plataforma móvil (iOS, Android, etc.), desactive el Módulo de entrada independiente para que el gamepad no pueda hacerse cargo
            ...
            // Ocultar el final del juego, la victoria y pantallas de imágenes más grandes
            ...
            // Asignar el ícono del temporizador y el texto para un acceso más rápido
            ...
            // Si tenemos un temporizador animado, asígnelo para un acceso más rápido
            ...
            // Registrar el número predeterminado de preguntas por grupo, para que cuando cambiemos el número de jugadores podamos actualizar este valor correctamente
            defaultQuestionsPerGroup = questionsPerGroup;

            // Update the current list of players based on numberOfPlayers
            // Actualiza la lista actual de jugadores según numberOfPlayers
            ...
            // Desactive el botón de la pregunta para que no intentemos abrir accidentalmente una imagen que no está allí.
            ...
            // Busque el objeto "Responder" que contiene todos los botones de respuesta y asigne todos los botones de respuesta a una matriz para facilitar el acceso
            if (GameObject.Find("Answers"))
            {
                // Define the answers holder which we will fill with the answer buttons
                // Definir el contenedor de respuestas que llenaremos con los botones de respuesta
                Transform answersHolder = GameObject.Find("Answers").transform;
                //Debug.Log(answersHolder);
                ...
                // Asignar los botones de respuesta
            ...
            //Si tenemos un lienzo de inicio, pausar el juego y mostrarlo. De lo contrario, simplemente inicie el juego.
            if (startCanvas)
            {
            ...
            else
            {
                ...
                // ¡Comenzar el juego! Configure la lista de preguntas (mezcle) y haga la primera pregunta
        ...
        /// Prepara la lista de preguntas e inicia el juego.También carga las preguntas XML desde una dirección en línea, si existe.
        /// </summary>
        public IEnumerator StartGame()
        {
            ...
            // Registrar la hora de inicio del juego para que podamos verificar cuánto tiempo tomó el cuestionario al final del juego
            ...
            // El índice de la primera pregunta en la lista de preguntas es en realidad -1, por lo que ajustamos el número del componente (1 se convierte en -1, 2 se convierte en 0, 10 se convierte en 8, etc.)
            ...
            // Asegúrese de que el límite de preguntas no sea mayor que el número real de preguntas disponibles
            ...
            // Asignar el objeto de progreso y todos los objetos relacionados para facilitar el acceso
            if (GameObject.Find("ProgressObject"))
            {
                ...
                if (GameObject.Find("ProgressObject/Tab"))
                {
                    ...
                    // Calcula el tamaño de una sola pestaña. Esto se usa para alinear la barra de progreso con el centro
                    progressTabSize = progressObject.GetComponent<GridLayoutGroup>().cellSize.x;

                    // Deactivate the original tab object
                    // Desactivar el objeto de pestaña original
                ...
                // Si tenemos un objeto de texto en el objeto de progreso, asígnelo y establezca el número de la primera pregunta
                if (GameObject.Find("ProgressObject/Text"))
                {
                    // Assign the text object
                    // Asignar el objeto de texto
                    progressTextObject = GameObject.Find("ProgressObject/Text");

                    // Reset the question limit counter
                    // Restablecer el contador de límite de preguntas
                    questionLimitCount = 0;

                    // Update the question count in the text
                    // Actualiza el recuento de preguntas en el texto
            ...
            // Comprueba si tenemos un componente de categoría adjunto a este controlador de juego
            if (GetComponent<Category>())
            {
                // Override the current questions list with the questions from the attached category
                // Anula la lista de preguntas actual con las preguntas de la categoría adjunta
                ...
                // Obtén el nombre de la categoría que cargamos. Esto se usa para verificar si completamos la categoría
            ...
            else if (GetComponent<CategoryMath>())
            {
                ...
                // Anula la lista de preguntas actual con las preguntas de la categoría matemática adjunta
            ...
            // Espera el siguiente cuadro. Esta línea se agregó porque la función necesita devolver algo o no funcionará.
            ...
            // Repase todas las preguntas y sobrescriba sus valores de texto de seguimiento con los valores de todo el cuestionario, si existen
            ...
            // Repase todas las preguntas y sobrescriba sus valores de imagen de seguimiento con los valores de todo el cuestionario, si existen
            ...
            // Repase todas las preguntas y sobrescriba sus valores de sonido de seguimiento con los valores de todo el cuestionario, si existen
            ...
            // Repase todas las preguntas y sobrescriba sus valores de tiempo con los valores de todo el cuestionario, si existen
            ...
            // Repase todas las preguntas y sobrescriba sus valores de bonificación con los valores de todo el cuestionario, si existen
            if (quizBonus > 0) foreach (Question question in questions) question.bonus = quizBonus;

            // Set the list of questions for this match
            // Establecer la lista de preguntas para esta coincidencia
            SetQuestionList();
        ...
        /// La actualización se llama cada fotograma, si MonoBehaviour está habilitado.
        /// </summary>
        void Update()
        {
            // Move the progress object so that the current question is centered in the screen
            // Mueve el objeto de progreso para que la pregunta actual esté centrada en la pantalla
            ...
            if (isGameOver == false)
            {
                ...
                // Si usamos el teclado o el gamepad, keyboardControls surten efecto
                ...
                // Si movemos el mouse en cualquier dirección o hacemos clic en él, o tocamos la pantalla en un dispositivo móvil, entonces los controles del teclado / gamepad se pierden
                ...
                // Cuenta atrás el tiempo hasta que acabe el juego
            ...
            // Actualiza el tiempo de reproducción del sonido
        ...
        /// 
        /// Establece la lista de preguntas, primero barajándolas y luego clasificándolas por valor de bonificación,
        /// y finalmente elegir un número limitado de preguntas de cada grupo de bonificación
        /// </summary>
        void SetQuestionList()
        {
            // Shuffle all the available questions
            // Mezcla todas las preguntas disponibles
            ...
            // Clasifica las preguntas en grupos según la bonificación que dan, de menor a mayor
            ...
            else
            {
                ...
                // Si no estamos ordenando por grupos de bonificación, entonces no es necesario limitar el número de preguntas por grupo
        ...
        /// 
        /// Mezcla la lista de preguntas especificada y la devuelve
        /// </summary>
        ...
        /// Mezcla la lista de respuestas especificada y la devuelve
        /// </summary>
        ...
        Answer[] ShuffleAnswers(Answer[] answers)
        {
            ...
            // Revisa todas las respuestas y barajalas
        ...
        /// Presenta una pregunta de la lista, junto con posibles respuestas.
        /// </summary>
        IEnumerator AskQuestion(bool animateQuestion)
        {
            if (isGameOver == false)
            {
                ...
                // Este booleano se usa para verificar si ya hicimos esta pregunta y luego hacer otra en su lugar
                bool questionIsUsed = false;

                // We are now asking a question
                // Ahora estamos haciendo una pregunta
                askingQuestion = true;

                // If we asked enough questions, move on to the next bonus group
                // Si hicimos suficientes preguntas, pase al siguiente grupo de bonificación
                if (questionCount >= questionsPerGroup)
                {
                    ...
                    // Desplácese por las preguntas hasta encontrar una pregunta con una bonificación más alta
                    while (tempBonus >= questions[currentQuestion].bonus)
                    {
                        ...
                        // Si nos quedamos sin preguntas, deja de revisar
                        ...
                        // Compruebe si la pregunta ya se ha utilizado y, si es así, haga otra pregunta
                ...
                else
                {
                    ...
                    // Si llegamos a la última pregunta del cuestionario, verifique si hay preguntas sin usar que podamos volver a poner en el cuestionario
                ...
                if (questionIsUsed == false)
                {
                    ...
                    if (currentQuestion < questions.Length)
                    {
                        ...
                        // Mezclar la lista de respuestas
                        ...
                        // Muestra las respuestas incorrectas y correctas en los espacios de respuesta
                        for (index = 0; index < questions[currentQuestion].answers.Length; index++)//  answerObjects.Length; index++)
                        {
                            ...
                            // Si el objeto de respuesta está inactivo, actívelo
                            ...
                            // Habilita el botón para que podamos presionarlo
        ...
        public void InsertBien()
        {
            ...
            codigo.CommandText = ("INSERT INTO ulearnet_reim_pruebas.alumno_respuesta_actividad (id_per, id_user, id_reim, id_actividad, id_elemento, fila, columna, correcta, datetime_touch)" +
                              " VALUES (" + id_periodo + ", " + Id_User + ", " + id_reim + ", " + actividad_id + "," + IdElemento + ", " + "1" + ", " + "1" + ", " + "1" + ", " + "'" + System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + "'" + ")");
            //Debug.Log();
